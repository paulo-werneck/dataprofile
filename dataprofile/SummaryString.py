...
class SummaryString(Summary):
    ...
    def get_summary_spark(self):
        a = self.data_frame.select(self.get_variables_segregated()[1]).summary("count", "min", "max"). \
            toPandas()
    ...
    def get_count_distinct(self):
        a = self.data_frame.agg(*(countDistinct(c).alias(c) for c in self.get_variables_segregated()[1])). \
            toPandas().transpose()
        final = a.reset_index()
    ...
    def get_missing(self):
        nulls = self.data_frame.select(
            *(when(isnull(c) == 'true', 1).otherwise(0).alias(c) for c in self.get_variables_segregated()[1]))
        agrupaded = nulls.groupBy().sum(*self.get_variables_segregated()[1])
        final = agrupaded.select(*(agrupaded['sum(' + c + ')'].alias(c) for c in self.get_variables_segregated()[1])). \
            toPandas().transpose()
